...
public class CloseBtn{
	...
	//x,yはメートル換算
	public CloseBtn(CCLayer layer, int ptmRatio, int baseLevel,
	...
	private void init(float x, float y, float size){
		...
		/////////////////////
        //背景のスプライトシート
		CCSpriteSheet ssBack = CCSpriteSheet.spriteSheet("close_btn.png", 100);
		layer.addChild(ssBack, 0, TAG_BACK);
		
		CGRect rect = CGRect.make(0, 0, 90, 90);
	...
	public void on(){
		if(!active){
			active = true;
			CGRect activeRect = CGRect.make(90, 0, 90, 90);
	...
	public void off(){
		if(active){
			active = false;
			CGRect inactiveRect = CGRect.make(0, 0, 90, 90);
	...
	public boolean isInside(CGPoint point){
		...
		if(innerRect.contains(point.x, point.y) && backSprite.getVisible()){
			return true;
		}else{
			return false;
		}
