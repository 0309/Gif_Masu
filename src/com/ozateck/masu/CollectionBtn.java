...
public class CollectionBtn{
	...
	//x,yはメートル換算
	public CollectionBtn(CCLayer layer, int ptmRatio, int baseLevel,
	...
	private void init(float x, float y, float size){
		...
		/////////////////////
        //背景のスプライトシート
		CCSpriteSheet ssBack = CCSpriteSheet.spriteSheet("collection_btn.png", 100);
		layer.addChild(ssBack, 0, TAG_BACK);
		
		CGRect rect = CGRect.make(0, 0, 330, 80);
	...
	public void on(){
		if(!active){
			active = true;
			CGRect activeRect = CGRect.make(0, 80, 330, 80);
	...
	public void off(){
		if(active){
			active = false;
			CGRect inactiveRect = CGRect.make(0, 0, 330, 80);
